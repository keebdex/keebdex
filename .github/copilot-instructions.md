...
- `KeysetForm.vue`'s Status/Review Status fields are controlled by a separate `showStatusFields` computed (`isStandalone || (moderator && isEdit)`), distinct from `showAdminFields` (`isStandalone`, still used for other admin-only UI). Pass both `:moderator` and `:is-edit` down from any embedding composite so staff reviewing an embedded/edit-mode form can see and change `review_status`; regular users never see these fields when creating a new keyset. The Review Status select is disabled (not hidden) unless `moderator` is true.
